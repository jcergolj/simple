...
class ReportProjectFilterTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_project_dropdown_contains_all_projects(): void
    {
        ...
        // All projects should be in the dropdown (JavaScript handles filtering)
        $response = $this->actingAs($user)->get(route('reports.index', [
            'client_id' => $client1->id,
        ]));

        $response->assertOk();
        $response->assertSee('Client One Project');
        $response->assertSee('Client Two Project');
    }
