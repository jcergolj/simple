<x-layouts.app :title="__('Reports')">
                ...
                <div class="flex flex-wrap gap-2 justify-between items-center">
                    ...
                    <div class="flex gap-6 text-sm">
                        ...
                        <div class="text-center">
                            ...
                            <div class="text-lg font-bold text-gray-900">${{ number_format($totalEarnings, 2) }}</div>
                        </div>
                    </div>
                </div>
            </form>
    ...
    <script>
        // Store all projects for filtering (avoid redeclaration)
        if (typeof window.allProjects === 'undefined') {
            window.allProjects = @json($projects->map(fn($p) => ['id' => $p->id, 'name' => $p->name, 'client_id' => $p->client_id]));
        }

        // Handle client selection change (avoid duplicate listeners)
        if (!window.reportsClientListenersAdded) {
            document.querySelectorAll('select[name="client_id"]').forEach(function(clientSelect) {
                clientSelect.addEventListener('change', function() {
                    ...
                    projectSelects.forEach(function(projectSelect) {
                        const currentProjectId = projectSelect.value;

                        // Clear current options except "All Projects"
                        projectSelect.innerHTML = '<option value="">{{ __('All Projects') }}</option>';

                        // Filter and add projects
                        const filteredProjects = selectedClientId
                            ? window.allProjects.filter(p => p.client_id == selectedClientId)
                            : window.allProjects;
            ...
            window.reportsClientListenersAdded = true;
        }

        // Handle date range selection (avoid duplicate listeners)
        const dateRangeSelect = document.getElementById('date-range-select');
        if (dateRangeSelect && !dateRangeSelect.hasAttribute('data-listener-added')) {
            ...
            dateRangeSelect.setAttribute('data-listener-added', 'true');
        }
    </script>
</x-layouts.app>
